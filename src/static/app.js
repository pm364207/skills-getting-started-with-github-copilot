document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = details.participants && details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map((participant) => `<li><span class="participant-email">${participant}</span><button class="delete-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}" aria-label="Remove ${participant}">✕</button></li>`).join("")}</ul>`
  ...
  // Delete participants on click (event delegation)
